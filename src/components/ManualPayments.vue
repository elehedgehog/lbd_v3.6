  ...
  export default {
      ...
      methods:{
        ...
        getQueryUserAccountList() {  //获取银行信息
          ...
          this.$http.get(this.lbdUtil.host +'/user/userAccount/queryUserAccountList?' + webInfo,
			    {params}
          ).then((res)=>{
        ...
        getVerifyCode(){ //获取验证码
          ...
          let webInfo = this.lbdUtil.getWebInfoUrl();
          let params  = this.$route.query;
          params.transactionNo = this.transactionNo
          this.$http.get(
          //  this.lbdUtil.host +'/payment/reapalRepay/getVerifyCode?' + webInfo.substr(1) + `&increaseBank=1&reservedMobile=${this.telephone}&userAccountId=${this.userAccountId}&transactionNo=${this.transactionNo}`,
           this.lbdUtil.host +'/payment/reapalRepay/getVerifyCode?' + webInfo.substr(1) + `&increaseBank=1&reservedMobile=${this.telephone}&userAccountId=${this.userAccountId}`,
           {params}
          ).then((res)=>{
        ...
        getConfirmPay(){ //确认还款点击确定之后执行
          ...
          let params  = this.$route.query;
          params.transactionNo = this.transactionNo
          // this.$http.get(this.lbdUtil.host +`/payment/reapalRepay/confirmPay?transactionNo=${this.transactionNo}&smsCode=${this.verificationCode}`+ webInfo,
          this.$http.get(this.lbdUtil.host +`/payment/reapalRepay/confirmPay?smsCode=${this.verificationCode}`+ webInfo,
          {params}
          ).then((res)=>{
             if(res.data.status==='0000'){
